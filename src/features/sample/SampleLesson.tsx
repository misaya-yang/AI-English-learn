    ...
    <section className="grid gap-6 lg:grid-cols-[minmax(0,0.95fr)_minmax(0,1.05fr)] lg:items-start">
      <div className="space-y-5">
        ...
        <div className="border-y border-border/20 py-5">
          ...
          <p className="mt-3 border-l border-border/20 bg-muted/35 px-3 py-2 text-sm leading-6 text-muted-foreground">
      ...
      <div className="border-y border-border/24 py-5 sm:py-6">
        <form className="space-y-5" onSubmit={handleSubmit}>
          ...
          <div className="flex flex-col gap-3 sm:flex-row">
            ...
            {hasFeedback ? (
              <Button type="button" variant="glass" className="rounded-full" onClick={handleTryAgain}>
        ...
        {hasFeedback ? (
          <div
            data-testid="sample-feedback"
	            className={cn('mt-5 border-l px-4 py-3', feedbackTone)}
            role="status"
          >
                ...
                {isCorrect ? (
                  <>
                    <div className="mt-4 grid gap-2 sm:grid-cols-3">
                      {loopItems.map((item) => (
	                        <span key={item} className="border-l border-border/20 bg-background/50 px-3 py-2 text-xs font-medium text-foreground">
                          {item}
                        </span>
                      ))}
                    </div>
