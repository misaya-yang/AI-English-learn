...
export function buildWordShareCardSvg(word: WordData, options: WordShareCardOptions): string {
  ...
  <text x="96" y="112" fill="#136f63" font-family="Inter, Arial, sans-serif" font-size="28" font-weight="700">VocabDaily</text>
  <text x="96" y="154" fill="#5f6f69" font-family="Inter, Arial, sans-serif" font-size="24">${escapeXml(subtitle)} · ${escapeXml(options.dateLabel)}</text>
  <text x="96" y="284" fill="#10201b" font-family="Georgia, serif" font-size="96" font-weight="700">${escapeXml(word.word)}</text>
  <text x="100" y="330" fill="#55716a" font-family="Inter, Arial, sans-serif" font-size="28">${escapeXml(word.partOfSpeech)} · ${escapeXml(word.phonetic)} · ${escapeXml(word.level)}</text>
  <foreignObject x="96" y="360" width="780" height="96">
    <div xmlns="http://www.w3.org/1999/xhtml" style="font-family: Inter, Arial, sans-serif; font-size: 30px; line-height: 1.35; color: #26332f;">${escapeXml(definition)}</div>
  </foreignObject>
  <foreignObject x="96" y="466" width="820" height="62">
    <div xmlns="http://www.w3.org/1999/xhtml" style="font-family: Inter, Arial, sans-serif; font-size: 22px; line-height: 1.35; color: #53615d;">${escapeXml(exampleText)}</div>
  </foreignObject>
  <rect x="930" y="350" width="150" height="42" rx="10" fill="#136f63" opacity="0.12"/>
  <text x="954" y="378" fill="#136f63" font-family="Inter, Arial, sans-serif" font-size="20" font-weight="700">${escapeXml(word.topic || 'general')}</text>
  <rect x="930" y="414" width="190" height="42" rx="10" fill="#d48b25" opacity="0.14"/>
  <text x="954" y="442" fill="#8b5a13" font-family="Inter, Arial, sans-serif" font-size="20" font-weight="700">${escapeXml(collocation)}</text>
  <text x="96" y="548" fill="#136f63" font-family="Inter, Arial, sans-serif" font-size="22">${isZh ? '公开分享卡，不包含个人学习历史' : 'Public share card. No private study history included.'}</text>
</svg>`;
}
