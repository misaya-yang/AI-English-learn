...
export const buildMissionCard = (args: {
  ...
  const primaryAction = recoveryMode
    ...
    : examPriority && (sprintMode || topWeakness?.emphasis === 'urgent' || !hasDailyWordsLeft)
      ? buildAction({
          ...
          surface: 'exam',
          title: args.recommendedUnitTitle ? `Boost your score with ${args.recommendedUnitTitle}` : 'Do one score-boost writing drill',
          titleZh: args.recommendedUnitTitle ? `先用《${args.recommendedUnitTitle}》做一次提分训练` : '先做一次提分写作练习',
    ...
    : hasDailyWordsLeft && !stretchMode
      ? buildAction({
          ...
          description: `${cefrCopy.descriptionPrefix.en} ${Math.max(vocabTarget - args.learnedTodayCount, 0)} suggested words are still waiting.`,
          descriptionZh: `${cefrCopy.descriptionPrefix.zh} 还有 ${Math.max(vocabTarget - args.learnedTodayCount, 0)} 个建议新词待学习。`,
          cta: 'Continue today',
          ctaZh: '继续学习',
  ...
  const secondaryActions: NextBestAction[] = [
    buildAction({
      id: 'secondary-coach',
      surface: 'chat',
      title: 'Open a guided practice session',
      titleZh: '打开一轮引导练习',
      description: 'Use guided help for explanations, collocations, or a short adaptive quiz.',
      descriptionZh: '做解释、搭配练习或一轮短测。',
      cta: 'Open practice',
      ctaZh: '打开练习',
      href: '/dashboard/chat',
      estimatedMinutes: 10,
