...
| 2026-06-17 | generator/evaluator | VD-04 | Added the first original IELTS Anki-style card foundation: 12-card deck, card schema, WordData and built-in word book mapping, vocabulary-bank entry point, Practice URL focus, Review manual URL focus, focused tests, and vocabulary browser assertion. | `reports/vd-04-ielts-anki-card-foundation-report.md`; `npm run lint`; `npm run check:i18n`; `npm run build`; `npm test -- --run`; focused tests 18/18; local learning-flow 160/160; `product-audit-2026-06-17/vd-04-learning-flow/summary.json` | Commit, push, deploy, run production smoke, and record production evidence. |
...
- VD-04 is locally passing. Final release work remains: commit, push, Vercel production deploy, production smoke, and production evidence writeback.
