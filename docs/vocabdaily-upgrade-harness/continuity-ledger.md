...
- Next action: Execute `supabase/migrations/20260617153000_auth_profile_bootstrap_rls.sql` only after user confirmation, then run `AUTH_FLOW_ACCOUNTS=3 npm run smoke:prod:auth-flow` and require both `functionalPassed` and `dbBootstrapPassed` before returning to VD-03.
